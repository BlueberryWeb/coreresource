 <!-- FOOTER -->
 <footer>
    <div class="d-none d-sm-none d-md-block" >
        <section class="showcase-footer" id="showcase">
            <div class="container  txt-footer">
                <div class="row pt-5">
                    <div class="text-center mb-5">
                        <img loading="lazy" src="{{ asset('front/img/logo-bco.gif') }}" alt="CORE" class="img-fluid logo-footer-bco"/>
                    </div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-1"></div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-10">
                        <div class="row">
                            <div class="col-12 col-sm-12 col-md-12 col-lg-4 mb-3">
                                <div class="me-5">
                                    <img src="{{ asset('front/img/logo-azul.gif') }}" alt="CORE" class="img-fluid mb-4 logo-footer"/>
                                    <p class="gRegular mb-0">We are a company who has been</p>
                                    <p class="gRegular my-0">giving global human resources</p>
                                    <p class="gRegular my-0">solutions to many countries for</p>
                                    <p class="gRegular mt-0">more than 10 years. </p>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-4 mb-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">SITE MENU</h3>
                                    <a href="{{ url('/') }}#index" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Index</h6>
                                    </a>
                                    <a href="{{ url('/') }}#who-we" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Who we are?</h6>
                                    </a>
                                    <a href="{{ url('/') }}#services" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Services</h6>
                                    </a>
                                    <a href="{{ url('/') }}#benefits" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Benefits</h6>
                                    </a>
                                    <a href="{{ url('/') }}#how-does" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">How does it work?</h6>
                                    </a>
                                    <a href="{{ url('/') }}#contact" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Contact</h6>
                                    </a>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-4 mb-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">LEGAL</h3>
                                    <a href="{{ route('privacy.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Privacy Policy</h6>
                                    </a>
                                    <a href="{{ route('terms.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Terms & Conditions</h6>
                                    </a>
                                    <a href="{{ route('cookies.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular mb-3">Cookies Policy</h6>
                                    </a>
                                </div>
                            </div>
                            <!--<div class="col-12 col-sm-12 col-md-12 col-lg-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">FOLLOW US</h3>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/fb-icono.webp') }}" alt="FB: Core" class="img-fluid icono-redes-foot me-3"/>
                                    </a>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/ig-icono.webp') }}" alt="IG: Core" class="img-fluid icono-redes-foot me-3"/>
                                    </a>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/tw-icono.webp') }}" alt="TW: Core" class="img-fluid icono-redes-foot"/>
                                    </a>
                                </div>
                            </div>-->
                        </div>
                    </div>
                    <div class="row pt-3">
                    <div class="col-12 col-sm-12 col-md-12 col-lg-1"></div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4">
                            <div class="">
                                <h6 class="gRegular">© 2022 Core Resources, All rights reserved</h6>
                            </div>
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4"></div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-2">
                            <img src="{{ asset('front/img/materialized-blueberry.gif') }}" alt="Materialized by Blueberry" class="img-fluid bb-logo"/>
                        </div>
                    </div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-1"></div>
                    
                </div>
            </div>
            <video src="{{ asset('front/vid/bg-footer.mp4') }}" playsinline autoplay muted loop></video>
        </section>
     </div>
      <!-- VISTA CELULAR -->
      <div class="d-block d-sm-block d-md-none" >
        <section class="showcase" id="showcase">
            <div class="container  txt-footer-cel">
                <div class="row pt-5">
                    <div class="text-center mb-5">
                        <img loading="lazy" src="{{ asset('front/img/logo-bco.gif') }}" alt="CORE" class="img-fluid logo-footer-bco"/>
                    </div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-1"></div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-10">
                        <div class="row">
                            <div class="col-12 col-sm-12 col-md-12 col-lg-3 mb-3">
                                <div class="me-5">
                                    <img src="{{ asset('front/img/logo-azul.gif') }}" alt="CORE" class="img-fluid mb-4 logo-footer"/>
                                    <p class="gRegular">We are a company who has been</p>
                                    <p class="gRegular">giving global human resources</p>
                                    <p class="gRegular">solutions to many countries for</p>
                                    <p class="gRegular">more than 10 years. </p>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-3 mb-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">SITE MENU</h3>
                                    <a href="{{ url('/') }}#index" class="a_none_style text-white">
                                        <h6 class="gRegular">Index</h6>
                                    </a>
                                    <a href="{{ url('/') }}#who-we" class="a_none_style text-white">
                                        <h6 class="gRegular">Who we are?</h6>
                                    </a>
                                    <a href="{{ url('/') }}#services" class="a_none_style text-white">
                                        <h6 class="gRegular">Services</h6>
                                    </a>
                                    <a href="{{ url('/') }}#benefits" class="a_none_style text-white">
                                        <h6 class="gRegular">Benefits</h6>
                                    </a>
                                    <a href="{{ url('/') }}#how-does" class="a_none_style text-white">
                                        <h6 class="gRegular">How does it work?</h6>
                                    </a>
                                    <a href="{{ url('/') }}#contact" class="a_none_style text-white">
                                        <h6 class="gRegular">Contact</h6>
                                    </a>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-3 mb-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">LEGAL</h3>
                                    <a href="{{ route('privacy.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular">Privacy Policy</h6>
                                    </a>
                                    <a href="{{ route('terms.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular">Terms & Conditions</h6>
                                    </a>
                                    <a href="{{ route('cookies.index') }}" class="a_none_style text-white">
                                        <h6 class="gRegular">Cookies Policy</h6>
                                    </a>
                                </div>
                            </div>
                            <!--<div class="col-12 col-sm-12 col-md-12 col-lg-3">
                                <div class="">
                                    <h3 class="mBold  h5 mb-3">FOLLOW US</h3>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/fb-icono.webp') }}" alt="FB: Core" class="img-fluid icono-redes-foot me-3"/>
                                    </a>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/ig-icono.webp') }}" alt="IG: Core" class="img-fluid icono-redes-foot me-3"/>
                                    </a>
                                    <a href="" class="a_none_style" target="_blank" rel="noopener noreferrer">
                                        <img src="{{ asset('front/img/tw-icono.webp') }}" alt="TW: Core" class="img-fluid icono-redes-foot"/>
                                    </a>
                                </div>
                            </div>-->
                        </div>
                    </div>
                    <div class="row pt-5">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4">
                            <div class="">
                                <h6 class="gRegular">© 2022 Core Resources, All rights reserved</h6>
                            </div>
                        </div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4"></div>
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4">
                            <img src="{{ asset('front/img/materialized-blueberry.gif') }}" alt="Materialized by Blueberry" class="img-fluid bb-logo"/>
                        </div>
                    </div>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-1"></div>
                    
                </div>
            </div>
            <video src="{{ asset('front/vid/bg-footer-vertical.mp4') }}" playsinline autoplay muted loop></video>
        </section>
     </div>
 </footer>


 <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.10.2/dist/umd/popper.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.min.js"></script>